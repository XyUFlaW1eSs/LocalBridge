...
界面仍是由用户浏览器打开的内嵌页面，尚无 WebView2/原生宿主窗口，因此不能把浏览器关闭按钮转换为“最小化到托盘”；该设置保留但暂不生效。
待接收/确认状态机已在 Sprint 4.2C 交付。下一个桌面切片必须实现原生宿主窗口，并完成交互式 Windows 托盘、注册表和 Explorer 冒烟测试，
之后才能宣称 Sprint 4.2 完整结束。
