...
| `v0.2.x` | Phase 2 | In progress | Sprints 2.1–2.9 delivered through TLS pinning and Windows protected credentials; production hardening remains |
...
### Phase 2 — Platform hardening, trust and discovery (in progress; Sprints 2.1–2.9 delivered)
...
- Sprint 2.6 fixes credential persistence, adds expiry and bounded-overlap rotation, provides a
  ...
  Sprint 2.9 subsequently protects those credentials at rest on Windows.
...
- Sprint 2.9 adds current-user Windows DPAPI for referenced management/pairing secrets, registry
  v4 protected peer-token persistence, atomic v3 migration, fail-closed startup and safe CLI
  set/delete/status operations. macOS Keychain and Linux Secret Service remain future work.
...
### Phase 4 — LAN collaboration modules (in progress; Sprint 4.2D delivered)
...
pending/approve/reject workflow when automatic receive is disabled. Sprint 4.2D adds a WebView2
host with tested close-to-tray and tray-restore behavior; the browser remains a documented fallback.
