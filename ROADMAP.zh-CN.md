...
- Phase 2 Sprint 2.1–2.9：请求身份、能力、认证、显式配对、发现、对端健康/出站传输、会过期的 peer Token 轮换、TLS 固定、版本化诊断和 Windows 受保护凭据。
- Phase 4.1：持久化多文件分享、移动端下载/上传页面、HTTP Range、Content-Range 续传和可恢复文件存储基础。
- Phase 4.2A–4.2D：内嵌文件 GUI、本地二维码 PNG、受控浏览器上传、当前用户级 Windows 开机启动/Explorer 同步、原生托盘、完成事件、接收确认和 WebView2 宿主。

## 计划版本

- **Phase 2 / `v0.2.x` — 平台加固：** Sprint 2.1–2.9 已交付至 TLS 固定和 Windows 受保护凭据；剩余工作包括面向用户的配置、
  非 Windows 受保护存储、共享重试/超时策略和安全默认迁移。
- **Phase 3 / `v0.3.x` — 同步引擎与富剪贴板：** 通用信封、能力协商、出站投递、离线队列、历史、图片、HTML/RTF 和截图。
- **Phase 4 / `v0.4.x` — 局域网协作：** 文件传输、内嵌 GUI、Windows 外壳、接收确认和原生宿主窗口已实现；URL 推送、图片投递、
  组合上下文任务和更完整的传输状态仍待完成。
