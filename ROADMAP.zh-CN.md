...
- Phase 2 Sprint 2.1–2.6：请求身份、能力、认证、显式配对、发现、对端健康/出站传输，以及可跨重启、会过期的 peer Token 轮换。
...
- **Phase 2 / `v0.2.x` — 平台加固：** Sprint 2.1–2.6 已交付至 peer Token 生命周期；剩余工作包括受保护的凭据存储、
  面向用户的配置、安全传输、诊断、共享重试/超时策略和配置迁移。
